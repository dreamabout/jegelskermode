...
function mm_ci_manage_page()
{
    ...
    <ol>
        ...
        <li><?php _e('Click the button below and we&#8217;ll scan all of your posts for remote images', 'cache-images'); ?>
            (<em><?php _e('Button <strong>Scan</strong> will search only for images that are hotlinked (ie. used like in example <code>&lt;img src="http://example.com/picture.jpg" /&gt;</code>), while button <strong>Scan (including linked)</strong> will search for images that are only linked from this site (ie. ised like in example <code>&lt;a href="http://example.com/picture.jpg"&gt;example&lt;/a&gt;</code>). Use second button with caution!', 'cache-images'); ?></em>)
        </li>
    ...
    if (!function_exists('wp_smushit')) {
        ...
        <div class="wp-smushit-notice error">
            ...
            echo sprintf(__(" (<a href='%s'>read more about WP Smush.it</a>)", "cache-images"), "http://dialect.ca/code/wp-smushit/");?>
            <br/>

            <?php echo sprintf(__("<a href='%s' class='thickbox'>Install WP Smush.it</a>", "cache-images"), esc_url(admin_url('plugin-install.php?tab=plugin-information&plugin=wp-smushit&TB_iframe=true&width=600&height=550'))); ?>
            <br/>
    ...
    <form action="" method="post">
        ...
        <p class="submit">
            ...
            <input type="button" onClick="javascript:getdomains();" class="button" name="Submit"
                   value="<?php _e('Scan &raquo;', 'cache-images'); ?>"/>

            <input type="button" onClick="javascript:getalldomains();" class="button" name="Submit"
                   style="margin-left: 200px;"
                   value="<?php _e('Scan (including linked) &raquo;', 'cache-images'); ?>"/> (<a
                onClick="javascript:showexplanation();"
                href="#"><?php _e('what is difference?', 'cache-images'); ?></a>)

        <div id="explain-all-domains" class="notice"
             style="display:none"><?php _e('Button <strong>Scan</strong> will search only for images that are hotlinked (ie. used like in example <code>&lt;img src="http://example.com/picture.jpg" /&gt;</code>), while button <strong>Scan (including linked)</strong> will search for images that are only linked from this site (ie. ised like in example <code>&lt;a href="http://example.com/picture.jpg"&gt;example&lt;/a&gt;</code>). Use second button with caution!', 'cache-images'); ?></div>
    ...
    <div id="thumb"
         style="display:none"><?php printf(__('Last cached picture: %1$s', 'cache-images'), '<span id="thumb-img"></span>'); ?></div>
...
function cache_images_ajax()
{
    ...
    if ($action == "getlist") {
        ...
        foreach ($postid_list as $v) {
            ...
            //preg_match_all('#(https?://[^\s]+(?=\.(jpe?g|png|gif)))#i', $post[0]->post_content, $matches);
            //preg_match_all('#(https?://[\'^\s]+(\?=\.(jpe?g|png|gif)))#i', $post[0]->post_content, $matches);
            preg_match_all('#(https?://[^\s]+(?=\.(jpe?g|png|gif)))#i', $post[0]->post_content, $matches);
        ...
        foreach ($posts as $post) {
            $domains = cache_images_find_all_images($post->post_content);
        }
...
function cache_images_find_images($content)
{
    $domains = array();
...
function cache_images_find_all_images($content)
{
    $domains = array();
    //preg_match_all('#(http://[^\s]+(?=\.(jpe?g|png|gif)))#i', $content, $matches);
    //preg_match_all('#(https?://[^\s]+(?=\.(jpe?g|png|gif)))#i', $content, $matches);
    //preg_match_all('#(https?://[\'^\s]+(\?=\.(jpe?g|png|gif)))#i', $content, $matches);
    preg_match_all('#(https?://[^\s]+(?=\.(jpe?g|png|gif)))#i', $content, $matches);
    foreach ($matches[1] as $url) {
        $url = parse_url($url);
...
function cache_images_cache_image($url, $postid)
{
    ...
    if (strpos($url, 'blogspot.com') || strpos($url, 'blogger.com') || strpos($url, 'ggpht.com') || strpos($url, 'googleusercontent.com') || strpos($url, 'gstatic.com')) {
        ...
        // Problem solved with src, from google content with https:
        if (strpos($url, 'googleusercontent.com')) {
...
function cache_images_save_post($post_ID, $post)
{
    ...
    if ($cache_images_settings && $cache_images_settings == 1) {
        ...
        $domains = cache_images_find_images($post->post_content);
        if (!$domains) {
            return $post_ID;
        }
        ...
        foreach ($domains as $domain => $num) {
            ...
            foreach ($matches[1] as $i => $url) {
                if (strstr($url, "jegelskermode.s3.amazonaws.com") || strstr($url, get_option('siteurl') . '/' . get_option('upload_path')) || !strstr($url, $domain)) {
                    continue; // Already local
                }
                var_dump("Caching {$url}");
...
function cache_images_field_settings_form()
{
    ...
    <label><input name="cache_images_automatic_caching" id="cache_images_automatic_caching"
                  type="checkbox" value="1"

            <?php checked('1', get_option('cache_images_automatic_caching')); ?> /> <?php _e('Automatically cache images on post&#8217;s saving', 'cache-images'); ?>
    </label>
