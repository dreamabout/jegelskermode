...
class autoblogcron {
	...
	function __construct() {
		...
		$this->debug = true || get_autoblog_option('autoblog_debug', true);
	...
	function get_autoblogentries($timestamp) {
		...
		if(function_exists('is_multisite') && is_multisite()) {
			if(function_exists('is_plugin_active_for_network') && is_plugin_active_for_network('autoblog/autoblogpremium.php')) {
				$sql = $this->db->prepare( "SELECT * FROM {$this->autoblog} WHERE site_id IN (" . implode(',', $sites) . ") AND nextcheck < %d AND nextcheck > 0 ORDER BY nextcheck ASC limit 10", $timestamp );
			} else {
				$sql = $this->db->prepare( "SELECT * FROM {$this->autoblog} WHERE site_id IN (" . implode(',', $sites) . ") AND blog_id IN (" . implode(',', $blogs) . ") AND nextcheck < %d AND nextcheck > 0 ORDER BY nextcheck ASC limit 10", $timestamp );
			}
		} else {
			$sql = $this->db->prepare( "SELECT * FROM {$this->autoblog} WHERE site_id IN (" . implode(',', $sites) . ") AND blog_id IN (" . implode(',', $blogs) . ") AND nextcheck < %d AND nextcheck > 0 ORDER BY nextcheck ASC limit 10", $timestamp );
		}
	...
	function get_autoblogentriesforids($ids) {
		...
		$sql = $this->db->prepare( "SELECT * FROM {$this->autoblog} WHERE site_id IN (" . implode(',', $sites) . ") AND feed_id IN (0, " . implode(',', $ids) . ") ORDER BY nextcheck ASC limit 10", $timestamp );
	...
	function test_feed($feed_id, $ablog) {
		...
		for ($x = 0; $x < $max; $x++) {
			...
			$author = $this->findAuthor($ablog, $item);
	...
	function process_feed($feed_id, $ablog) {
		...
		$this->debug("Processing {$ablog['url']}\n");
		$feed = fetch_feed($ablog['url']);
	...
	*/
	...
	private function findAuthor($ablog, $item)
	{
		if($ablog['author'] == '0') {
			...
			$author = apply_filters("autoblog_author", $author, $ablog);
