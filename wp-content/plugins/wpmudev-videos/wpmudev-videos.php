...
class WPMUDEV_Videos {
	...
	function WPMUDEV_Videos() {
		...
		$this->video_cats = array(
			...
			'images' => array('name' => __('Working With Images', 'wpmudev_vids'),
												'list' => array('add-image-from-pc', 'add-image-from-media-library', 'add-image-from-url', 'image-gallery', 'edit-image', 'replace-image', 'delete-image', 'featured-image')),
			...
			'Comments' => array('name' => __('Managing Comments', 'wpmudev_vids'),
														'list' => array('comments')),
			'users' => array('name' => __('Users', 'wpmudev_vids'),
														'list' => array('create-edit-user'))
		);
	...
	function plug_pages() {
		...
		if (!is_network_admin()) {
			if ($this->get_setting('menu_location') == 'dashboard') {
				add_submenu_page( 'index.php', $this->get_setting('menu_title'), $this->get_setting('menu_title'), 'read', 'video-tuts', array( &$this, 'page_output') );
	...
	function settings_output() {
		...
		<div class="postbox">
			<h3 class='hndle' style="cursor:default;"><span><?php _e('Display Settings', 'wpmudev_vids') ?></span></h3>
			<div class="inside">
				<table class="form-table">
					<tr>
						...
						<td>
							...
							<input type="submit" class="button-primary" name="save_settings" value="<?php _e('Now Confirm Registration', 'wpmudev_vids') ?>" />
							<?php
								$site_id = '';
							} else {
								...
								<input name="wpmudev_vids[site_id]" value="<?php echo esc_attr($this->get_setting('site_id', $site_id)); ?>" type="hidden" />
							<?php } ?>
						</td>
					</tr>
					...
					<tr>
						...
						<td>
							<span class="description"><?php _e('For compatibility with your current Domain Mapping settings you must enter your <a href="http://premium.wpmudev.org/wp-admin/profile.php?page=vidapi" target="_blank">WPMU DEV API Key</a>:', 'wpmudev_vids'); ?></span><br />
							<label><input size="45" name="wpmudev_apikey" value="<?php echo esc_attr(get_site_option('wpmudev_apikey')); ?>" type="text" placeholder="<?php _e('Enter your WPMU DEV API Key', 'wpmudev_vids') ?>" /></label>
						</td>
					</tr>
					<?php } ?>
					<tr>
						...
						<td>
						<label><input value="1" name="wpmudev_vids[contextual_help]" type="radio"<?php checked($this->get_setting('contextual_help', 1), 1) ?> /> <?php _e('Yes', 'wpmudev_vids') ?></label>
					...
					<tr>
						...
						<select name="wpmudev_vids[menu_location]">
							<option value="dashboard"<?php selected($this->get_setting('menu_location'), 'dashboard') ?>><?php _e('Dashboard', 'wpmudev_vids') ?></option>
							<option value="top"<?php selected($this->get_setting('menu_location'), 'top') ?>><?php _e('Top Level', 'wpmudev_vids') ?></option>
							<?php if (function_exists('incsub_support')) { ?>
							<option value="support_system"<?php selected($this->get_setting('menu_location'), 'support_system') ?>><?php _e('Support System Plugin', 'wpmudev_vids') ?></option>
							<?php } ?>
				...
				<p class="submit">
				<input type="submit" class="button-primary" name="save_settings" value="<?php _e('Save Changes', 'wpmudev_vids') ?>" />
				</p>
			</div>
		</div>
	...
	function page_output() {
		...
		<div id="poststuff" class="metabox-holder">
			...
			<div class="postbox">
				<h3 class='hndle' style="cursor:default;"><span><?php esc_attr_e($this->video_list[$_GET['vid']]); ?></span></h3>
				<div class="inside">
					<iframe style="display:block;margin:0 auto;box-shadow:30px 0 50px -30px #222, -30px 0 50px -30px #222;" src="<?php echo $this->create_embed_url($_GET['vid'], true); ?>" frameborder="0" width="600" height="338" webkitAllowFullScreen mozallowfullscreen allowFullScreen></iframe>
				</div>
			</div>
			<?php } ?>

			<div class="postbox">
				<h3 class='hndle' style="cursor:default;"><span><?php _e('Select a Video Tutorial', 'wpmudev_vids') ?></span></h3>
...
$wpmudev_notices[] = array( 'id'=> 248,'name'=> 'WPMU DEV Videos', 'screens' => array( 'settings_page_wpmudev-videos', 'settings_page_wpmudev-videos-network', 'toplevel_page_video-tuts', 'dashboard_page_video-tuts' ) );
